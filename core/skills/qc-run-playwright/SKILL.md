...
| Input | Source |
|---|---|
| Test cases to run (IDs or scope) | `qc/test-cases.md` or `specs/*.feature` |
...
1. **Plan.** Build a run plan table: TC ID, title, steps summary, expected results, priority. Save it as `qc/qc-task.md` (append section "Playwright Run") with per-TC status `PENDING`.
...
7. **Cleanup.** Remove debug notes, temporary waits, and any credential traces from conversation artifacts. Update `qc/qc-task.md` with final per-TC status: `PASS (2/2)`, `FAIL`, `BLOCKED`, or `SKIPPED` with reason.
