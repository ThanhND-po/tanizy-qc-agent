## Tanizy QC Agent for Claude Code
...
- Keep installed skills in `.claude/skills/qc-*` and project-owned artifacts under `qc/`. Never install skills inside `qc/`.
...
Route only to a `qc-*` skill that exists in `.claude/skills/`.
