...
Keep these run-control and execution columns editable:

```text
Selected for Run, Attempt, Test Result, Actual Result, Tested By, Tested At, Evidence, Defect, Cleanup, Note
```
...
CSV uses one row per attempt. Repeat Run metadata on every row because CSV has no separate metadata sheet. Require these headers:

```text
ScopeKey,ScopeCode,RunID,SourceTestCases,SourceRevision,Environment,
ApplicationBuild,ExecutionMethod,Executor,RetryPolicy,AssessmentPolicy,
CleanupPlan,EvidencePolicy,Attempt,TCID,TestResult,ActualResult,TestedBy,
TestedAt,Evidence,Defect,Cleanup,Note
```

Accept a UTF-8 CSV with a header row. Detect inconsistent repeated Run metadata as an import error. Blank `TestResult` rows do not create attempts.
...
```markdown
| Selected for Run | Attempt | TC ID | Test Result | Actual Result | Tested By | Tested At | Evidence | Defect | Cleanup | Note |
|---|---:|---|---|---|---|---|---|---|---|---|
| TRUE | 1 | TC-LOG-001 | | | | | | | | |
```
...
When a native spreadsheet skill is available:
...
If the native skill imposes a runtime-specific output directory, follow that skill and record the actual locator. Copy the workbook into the project only after separate path approval.
